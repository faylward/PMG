...
downloads = "/home/frankaylward/Desktop/marker_gene_benchmarking/metagenomes/metagenomes_5KB/proximity_test/test_metagenomes/"

merged = open("merged_COG0085_wCOG0086.txt", "w")
...
for i in path_to_gff:
			...
			if protein2dups[protein +"|"+ annot] == "SECO":
				...
				for d in orf_set:
					if d in protein2cog:
						...
						if protein2cog[d] == "COG0086":
							#print "COG0086"
							protein2adj[protein].append(d)
	...
	for item in protein2dups:
		if protein2dups[item] == "MAIN" or protein2dups[item] == "PRIMARY" or protein2dups[item] == "NEXT":
			...
			merged.write(acco +"_"+ protein +"\t"+ protein2acc[protein] +"\t"+ hit +"\t"+ protein2dups[item] +"\t"+ str(protein2length[protein]) +"\t"+ str(protein2score[protein]) +"\t"+ str(protein2align_length[item]) +"\t"+ str(num_proteins[item]) +"\t"+ prot_str +"\t"+ loc_str +"\t"+ adj_str +"\n")
