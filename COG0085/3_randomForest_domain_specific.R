...
bacteria <- row.names(x)[which(x$domain == "Bacteria")]
archaea <- row.names(x)[which(x$domain == "Archaea")]
...
fp <- dim(subset(final, final$predicted=="BH" & final$category == "NH" & row.names(final) %in% archaea))[1]
fn <- dim(subset(final, final$predicted=="NH" & final$category == "BH" & row.names(final) %in% archaea))[1]
tp <- dim(subset(final, final$predicted=="BH" & final$category == "BH" & row.names(final) %in% archaea))[1]
tn <- dim(subset(final, final$predicted=="NH" & final$category == "NH" & row.names(final) %in% archaea))[1]
...
bacteria <- row.names(set)[which(set$domain == "Bacteria")]
archaea <- row.names(set)[which(set$domain == "Archaea")]

passed <- subset(set, set$score >= 540)
removed <- subset(set, set$score < 540)

tp <- sum(passed$category == "PRIMARY" & row.names(passed) %in% archaea)
fp <- sum(passed$category != "PRIMARY" & row.names(passed) %in% archaea)
tn <- sum(removed$category != "PRIMARY" & row.names(removed) %in% archaea)
fn <- sum((removed$category == "PRIMARY" | removed$category == "SECO" | removed$category == "MAIN")  & row.names(removed) %in% archaea)
...
tp <- sum(passed$category == "PRIMARY" & row.names(passed) %in% bacteria)
fp <- sum(passed$category != "PRIMARY" & row.names(passed) %in% bacteria)
tn <- sum(removed$category != "PRIMARY" & row.names(removed) %in% bacteria)
fn <- sum((removed$category == "PRIMARY" | removed$category == "SECO" | removed$category == "MAIN")  & row.names(removed) %in% bacteria)
